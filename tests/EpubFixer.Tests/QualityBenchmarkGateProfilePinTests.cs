...
/// <summary>
/// Pins D80 kural 1: a gate threshold is the measured value of a passing engine, never a hand
/// picked number. This test does not hardcode any of docs/baselines/quality-gate.json's numbers -
/// it reads the legacy engine's measured counts from ocrStageMeasurement.legacy and the active
/// thresholds from current, then runs both through the real QualityBenchmarkGateEvaluator.
...
{
    [Fact]
    public void CurrentProfile_PassesAgainstLegacyMeasuredCounts()
    {
        ...
        var legacy = current.GetProperty("ocrStageMeasurement").GetProperty("legacy");

        var result = BuildResultFromMeasurement(legacy);
        ...
        Assert.True(
            gate.Passed,
            "Legacy's own measured counts must pass the gate's current thresholds - the thresholds " +
            "are supposed to be legacy's measured values (D80 kural 1), not something stricter. " +
